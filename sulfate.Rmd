...
div.main-container {
  max-width: 1200px !important;  /* widen document */
  margin-left: auto;
  margin-right: auto;
...
BRACKETING_REFERENCE <- "SodSul_2_ref"
...
# Standards you care about keeping consistent
STANDARD_NAMES <- c(
  BRACKETING_REFERENCE,
  "SodSul_2",
  "SodSul_4",
  "SodSul_8",
  "S_MIF_2"
...
# Choose distinct colors for standards
STANDARD_COLORS <- c(
...
# Choose distinct shapes for standards (21–25 are nice filled symbols)
STANDARD_SHAPES <- c(
  setNames(21, BRACKETING_REFERENCE),
  "SodSul_2" = 21,
  "SodSul_4" = 22,
  "SodSul_8" = 23,
  "S_MIF_2"  = 24
...
process_one_raw_file <- function(raw_file, high_res_directory) {
  ...
  inj$largest_peaks <- head(peaks_clustered,20)
...
Our initial code to calculate deltas from Ivan (a Thermo Exploris 240 Orbitrap Mass Spectrometer), Ruby (an HPLC), and Stella (a syringe and pump), all located in IsoLab, UW. This R Markdown grew from the R script "CalculateDeltas.R".
...
opts <- furrr::furrr_options(
  seed = TRUE,
  scheduling = 2, # if I/O heavy and file sizes vary, keep >=2
  packages  = c("dplyr","tidyr","purrr","ggplot2","fs","tools","stringr") # plus what you use
  # globals = c("ISOTOPOCULES","RUN_DIRECTORY","PEAK_DEFINITION_STRATEGY",
  #             "SAMPLE_PEAK_START","SAMPLE_PEAK_END")  # add if referenced inside process_one_injection()
)
...
run_figures[[fig_n]] <- list(
    ...
    plot = ggplot(ruby_data_summary) + 
        ...
        geom_point(aes(x=sample_order, y=TICmean, color=sample_name, shape=sample_name, fill=sample_name), size=6) +
        scale_color_manual(values=color_values) +
        scale_shape_manual(values=shape_values) +
        scale_fill_manual(values=color_values) +
...
run_figures[[fig_n]] <- list(
    ...
    plot = ggplot(ruby_data_summary) + 
        geom_errorbar(aes(x = sample_order, ymin = OICmean - OICsd, ymax = OICmean + OICsd, color = sample_name),
                      width = 0.2, linewidth = 1) +
        geom_point(aes(x=sample_order, y=OICmean, color = sample_name, shape=sample_name, fill=sample_name), size = 6) +
        scale_color_manual(values=color_values) +
        scale_shape_manual(values=shape_values) +
        scale_fill_manual(values=color_values) +
...
# iso_tbl <- tibble::tibble(
#   label = names(ISOTOPOCULES),
#   mz    = as.numeric(ISOTOPOCULES)
# )
# x_right <- max(largest_peaks_tbl$sample_order) * 1.02
# 
# title_text =  sprintf("Figure %d - mz vs sample order", fig_n)
# run_figures[[fig_n]] <- list(
#     title_text =  title_text,
#     anchor = sprintf("fig%d", fig_n),
#     caption_text = "Measured m/z versus sample order",
#     
#     plot = ggplot(largest_peaks_tbl,
#            aes(x = sample_order,
#                y = mean_mz,
#                size = 3,
#                color = sample_name)) +
...
#       #   name = "Peak intensity",
#       #   trans = "log10",
#       #   range = c(1, 8)   # tweak as you like
#       # ) +
#       labs(
#         x = "Sample order",
#         y = "Mean m/z",
#         color = "Sample name",
#         title = "Largest peaks over time"
#       ) +
#       geom_hline(data = iso_tbl,
#                  aes(yintercept = mz),
#                  linetype = "dashed",
#                  color = "gray40",
#                  linewidth = 0.5) +
#       geom_text(data = iso_tbl,
#                 aes(x = x_right, y = mz, label = label),
#                 hjust = -0.1, color = "gray20", size = 3) + 
...
for (rec in each_injection) {
  ...
  if (!is.null(rec$largest_peaks) && nrow(rec$largest_peaks) > 0) {
    ...
    cat('<div class="section-indent">Twenty largest peaks: <table style="border-collapse:collapse; margin-top:1rem;">\n')
    cat('<tr>',
...
Analyzer temperature is stored in the .raw data files. While we still dont know where exactly in the instrument this measurement originates, it does track with lab temperature. Furthermore, it also seems to explain much of the ratio variability. This Ruby-only version of the code does not yet have a way to flag temperature variability data like previous Stella vs Ruby scripts did.
...
message('\n ---------- find the nearest standard before and after for each sample ----------')
analyses <- analyses %>%
  rowwise() %>%
  mutate(
...
analyses <- analyses %>%
  left_join(
    bracketing_references %>%
      select(sample_order, analyzerTmean, TICmean) %>%
      rename(nearest_ref_before = sample_order, mean_temp_before = analyzerTmean, TIC_before = TICmean),
    by = "nearest_ref_before"
  ) %>%
  left_join(
    bracketing_references %>%
      select(sample_order, analyzerTmean, TICmean) %>%
      rename(nearest_ref_after = sample_order, mean_temp_after = analyzerTmean, TIC_after = TICmean),
...
message('\n ---------- wM0 - join to get R34S, R33S, R18O, and R17O for nearest before and after ----------')
analyses <- analyses %>%
  left_join(
    bracketing_references %>% 
...
Fig__R34S_vs_order <- ggplot() +
  geom_point(data = bracketing_references,
             mapping = aes(x=sample_order, y=R34S), color="black", shape=22, fill="green", size = 6) +
  geom_text(data = bracketing_references,
            mapping = aes(x=sample_order, y=R34S, label=sample_order), 
            color = "black", size = 2.5, fontface = "bold", vjust = 0.35) +
  geom_point(data = analyses,
             mapping = aes(x=sample_order, y=R34S, color=sample_name, shape=sample_name, fill=sample_name), size=6) +
  scale_color_manual(values=color_values) +
  scale_shape_manual(values=shape_values) +
  scale_fill_manual(values=color_values) +
  geom_text(data = analyses,
            mapping = aes(x = sample_order, y = R34S, label = sample_order), 
            color = "white",
            size = 2.5, fontface = "bold", vjust = 0.35) +
  labs(
    title = "R34S vs sample order.",
    x = "Sample order",
    y = "R34S"
  ) +
  theme_minimal() +
...
Fig__R34Sref_vs_order <- ggplot() +
  geom_point(data = bracketing_references,
             mapping = aes(x = sample_order, y = R34S), color="black", size=3, fill="green", shape=21) +
  geom_point(data = analyses, 
             mapping = aes(x = sample_order, y = R34S_ref, color = sample_name), size = 6) +
  geom_text(data = analyses,
            mapping = aes(x = sample_order, y = R34S_ref, label = sample_order),
...
Numbered symbols show the mean bracketing reference R34S. Smaller pink dots show the measured bracketing reference R34S.
...
```{r individual-replicate-assessment, echo=FALSE, fig.width=10, fig.height=6}
...
Fig_delta34S_vs_order <- ggplot(samples_df_order,
                                aes(x = x_pos, y = d34S_VCDT, color = sample_name)) +
  ...
  geom_point(size = 5) +
  geom_text(aes(label = sample_order), vjust = 0.5, hjust = 0.5, color = "black") +
  scale_x_continuous(breaks = NULL) +   # hide x labels
  labs(
    x = "Sample Type",
    y = expression(delta^34*"S (VCDT)"),
    color = "Sample Name"               # legend appears
  ) +
  theme_minimal() +
...
Fig_d34S_vs_TIC <- ggplot(run_summary_concentration, aes(x=TICmean, y=d34S_VCDT, color=sample_name)) +
                                    geom_point(size = 5) +
...
Fig_d34S_vs_TICratio <- ggplot(run_summary_concentration, aes(x=TICratio, y=d34S_VCDT, color=sample_name)) +
                                    geom_point(size = 5) +
                                    geom_errorbar(aes(ymin = d34S_VCDT - d34S_VCDT_95CI,
                                                    ymax = d34S_VCDT + d34S_VCDT_95CI),
                                                width = 0.2,
...
Fig_d34S_vs_concentration <- ggplot(run_summary_concentration, aes(x=concentration, y=d34S_VCDT, color=sample_name)) +
                                    geom_point(size = 5) +
                                    geom_errorbar(aes(ymin = d34S_VCDT - d34S_VCDT_95CI,
                                                    ymax = d34S_VCDT + d34S_VCDT_95CI),
                                                width = 0.2,
...
Fig_tic_vs_concentration <- ggplot(run_summary_concentration, aes(x=concentration, y=TICmean, color=sample_name)) +
                                    geom_point(size = 5) +
                                    geom_errorbar(aes(ymin = TICmean - TICsd,
                                                    ymax = TICmean + TICsd),
                                                width = 0.2,
                                                linewidth = 0.8) +
...
Fig_d34Sresidual_vs_order <- ggplot() + 
      geom_point(data=residuals, mapping=aes(x=sample_order, y=d34S_residual, color=sample_name), size=6) +
      geom_text(data = residuals, mapping = aes(x = sample_order, y = d34S_residual, label = sample_order)) +
...
Fig_d34Sresidual_vs_TIC <- ggplot() + 
      geom_point(data=residuals, mapping=aes(x=TICmean, y=d34S_residual, color=sample_name), size=6) +
...
Fig_d18Oresidual_vs_order <- ggplot() + 
      geom_point(data=residuals, mapping=aes(x=sample_order, y=d18O_residual, color=sample_name), size=6) +
