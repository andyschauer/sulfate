...
SAMPLES_TO_IGNORE <- c('SodSul-8_22', 'S-MIF-2_52', 'S-delta_72', 'S-bravo_50')          # if a particular sample is bad, enter its filename with quotes around it here, e.g. 'SodSul-2-ref_5'
...
raw_agg_file <- raw_files[1] |> orbi_read_raw(include_spectra = FALSE) |> orbi_aggregate_raw()
if (raw_agg_file$file_info$LowMass<80){
  # SO3
...
d34S_precision <- round(sd(residuals$d34S_residual, na.rm = TRUE) * 2, 2)
d18O_precision <- round(sd(residuals$d18O_residual, na.rm = TRUE) * 2, 2)
d34S_outlier <- round(sd(residuals$d34S_residual, na.rm = TRUE) * 3.3, 2)
d18O_outlier <- round(sd(residuals$d18O_residual, na.rm = TRUE) * 3.3, 2)


d34S_title <- paste0("d34S residual 2-sigma = ", d34S_precision, " permil; (yellow = 95%; red = 99.9%)")
Fig_d34Sresidual_vs_order <- ggplot(residuals) + 
      ...
      geom_hline(yintercept = d34S_precision,   linetype = "solid", color = "yellow",   linewidth = 0.5) +
      geom_hline(yintercept = -d34S_precision,   linetype = "solid", color = "yellow",   linewidth = 0.5) +
      geom_hline(yintercept = d34S_outlier,   linetype = "solid", color = "red",   linewidth = 0.5) +
      geom_hline(yintercept = -d34S_outlier,   linetype = "solid", color = "red",   linewidth = 0.5) +
      scale_color_manual(values=color_values) +
      scale_shape_manual(values=shape_values) +
      scale_fill_manual(values=color_values) +
...
d18O_title <- paste0("d18O residual 2-sigma = ", d18O_precision, " permil; (yellow = 95%; red = 99.9%)")
Fig_d18Oresidual_vs_order <- ggplot() + 
      ...
      geom_hline(yintercept = d18O_precision,   linetype = "solid", color = "yellow",   linewidth = 0.5) +
      geom_hline(yintercept = -d18O_precision,   linetype = "solid", color = "yellow",   linewidth = 0.5) +
      geom_hline(yintercept = d18O_outlier,   linetype = "solid", color = "red",   linewidth = 0.5) +
      geom_hline(yintercept = -d18O_outlier,   linetype = "solid", color = "red",   linewidth = 0.5) +
      scale_color_manual(values=color_values) +
      scale_shape_manual(values=shape_values) +
      scale_fill_manual(values=color_values) +
