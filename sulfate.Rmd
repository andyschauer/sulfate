...
d17O_CALIBRATION_STANDARDS <- c('SodSul_8', 'S_MIF_2') 
d18O_CALIBRATION_STANDARDS <- c('SodSul_8', 'S_MIF_2')
...
process_one_injection <- function(filename, df, high_res_directory) {
  ...
  # Step 2: if any duplicates, message the user
  ...
  # Step 3: drop duplicates (keep first row for each scan.no + isotopocule)
  df_nodups <- df %>%
    dplyr::group_by(scan.no, isotopocule) %>%
    dplyr::slice(1L) %>%        # or dplyr::distinct(scan.no, isotopocule, .keep_all = TRUE)
    dplyr::ungroup()
  
  # Your original block, now using df_nodups
  df_wide <- df_nodups %>%
    tidyr::pivot_wider(
      id_cols = scan.no,
  ...
  ruby_end_window <- which(df_wide$time.min>1 & df_wide$time.min<5)
  ...
  trimmed_window <- which(convolve_df$dtic_dT[ruby_start_index:ruby_end_index]<5e6 & convolve_df$dtic_dT[ruby_start_index:ruby_end_index]>-5e6)
  ...
  if (PEAK_DEFINITION_STRATEGY=='smoothed_derivative'){
    ...
    message(paste0("PEAK_DEFINITION_STRATEGY was entered as ", PEAK_DEFINITION_STRATEGY, " and is not recognized. Your options are 'smoothed_derivative', 'trimmed_median', 'combined', 'manual'"))
    knitr::knit_exit()
  }
...
process_one_raw_file <- function(raw_file, high_res_directory) {
  ...
  # main per-injection processing (your function)
  res <- process_one_injection(filename, isoraw, high_res_directory)

  # make M0/M1/M2 diagnostic scatter plots using res$scanlimits
  peaks <- subset(
    raw_agg_file$peaks,
    (scan.no > res$scanlimits[1]) & (scan.no < res$scanlimits[2])
  )
  ...
  extra_pngs <- purrr::map_chr(plots_df, function(x) {
    p <- ggplot(x$df) +
  ...
  res$largest_peaks <- head(peaks_clustered,10)

  res$images <- c(res$images, extra_pngs)
  res
...
plot_temperature <- function(samples_data_frame){
  Fig__AnalyzerT_vs_order <- ggplot() +
  geom_errorbar(data = samples_data_frame,
                mapping = aes(x = sample_order, 
                              ymin = analyzerTmean - analyzerTsd*2, 
                              ymax = analyzerTmean + analyzerTsd*2, 
                              color = sample_name),
                  width = 0.2,  # Controls the width of error bars
                  linewidth = 1) +   
  geom_point(data = samples_data_frame, 
             mapping = aes(x = sample_order, y = analyzerTmean, color = sample_name), 
             size = I(6)) +
  geom_text(data = samples_data_frame,
          mapping = aes(x = sample_order, 
                        y = analyzerTmean,
                        label = sample_order),
...
<details><summary class="expandable_subtitles">Data Processing Settings</summary>
  ...
  **Ideal Calibration Standards** - Agreed upon calibration standards as of 2025-05-22:
...
each_injection <- ok
rows           <- purrr::map(ok, "row")
ruby_data_summary <- dplyr::bind_rows(rows)

```

<details><summary class="expandable_subtitles">High Resolution Data</summary>
...
run_figures[[fig_n]] <- list(
    ...
    plot = ggplot(ruby_data_summary) + 
        ...
        geom_point(aes(x=sample_order, y=TICmean, color = sample_name), size = I(5)) +
...
run_figures[[fig_n]] <- list(
    ...
    plot = ggplot(ruby_data_summary) + 
        geom_errorbar(aes(x = sample_order, ymin = OICmean - OICsd, ymax = OICmean + OICsd, color = sample_name),
                      width = 0.2, linewidth = 1) +
        geom_point(aes(x=sample_order, y=OICmean, color = sample_name), size = I(5)) +
...
for (rec in each_injection) {
  ...
  cat(sprintf('Ten largest peaks: %s\n', rec$top10peaks))
...
geom_text(data = analyses,
        ...
        vjust = 0.5, hjust = 0.5, color = "black")+
labs(
  title = paste0("R34S vs sample order."),
  x = "Sample order",
  y = "R34S") +
theme_minimal() +
theme(
