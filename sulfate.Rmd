...
sd_data <- residuals %>%
  ...
  mutate(
    ymin = mean_residual - sd_residual,
...
Fig_d34Sresidual_vs_name <- ggplot() +
  ...
  # ---- Points ----
  geom_point(
    data = residuals_flagged,
    aes(x = sample_name, y = d34S_residual, color = sample_name),
    size = 6
  ) +
  
  # ---- Text labels (bold if outside SD band) ----
  geom_text(
    data = residuals_flagged,
    aes(
      x = sample_name,
  ...
  labs(
    title = "1-Sigma range highlighted. Points outside that range are bold",
    x = "Sample Name",
